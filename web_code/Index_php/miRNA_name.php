...
if ($autocomplete && !empty($term)) {
    ...
    switch ($species) {
        case 'Mouse':
            $sql = "SELECT `miRNA_name` FROM `20240812mouseaqseqnormalized` WHERE `miRNA_name` LIKE '%$term%'";
            break;
        case 'Human':
            $sql = "SELECT `miRNA_name` FROM `20240820humanaqseqnormalized` WHERE `miRNA_name` LIKE '%$term%'";
            break;
        case 'D.melanogaster':
            $sql = "SELECT `miRNA_name` FROM `2024082drosophilaaqseqnormalized` WHERE `miRNA_name` LIKE '%$term%'";
            break;
        case 'C.elegans':
            $sql = "SELECT `miRNA_name` FROM `20240816mirna_celegans_normalized` WHERE `miRNA_name` LIKE '%$term%'";
            break;
        default:
