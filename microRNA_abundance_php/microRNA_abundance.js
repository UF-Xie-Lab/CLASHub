...
function downloadCSV(data, miRNAName) {
    // UPDATED: New headers include the specific Replicate Source
    const rows = [['Cell Line / Tissue', 'Sample ID', 'Expression Value (CPM)']]; 

    // Loop through each Tissue/Group (e.g., N2_EE, Ebax_L1)
    Object.keys(data.data).forEach(tissue => {
        const values = data.data[tissue];            // Array of numbers: [47410.9, 45754.3]
        const sources = data.tissues[tissue] || [];  // Array of names: ['SRR...05', 'SRR...06']

        // Loop through each value in this group
        values.forEach((value, index) => {
            // Get the specific SRR name for this value (or 'N/A' if missing)
            const sourceName = sources[index] || 'N/A';
            
            // Add a distinct row for this specific replicate
            rows.push([tissue, sourceName, value]);
        });
    });
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            afterDraw: chart => {
                ...
                ctx.fillText('NT: Non-targeting', annotationX, annotationY + 20);
                ctx.fillText('Z8: Zswim8 Knockout', annotationX, annotationY + 40);

                // 绘制 "Data Source: GSE231447" 在最底部
                ctx.font = '12px Arial';
                ctx.fillText('Data Source: GSE235065, GSE158025, GSE160304, PRJNA1093144',  0,  y+40); // 绘制普通文本
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            afterDraw: chart => {
                ...
                ctx.font = '14px Arial';
                ctx.fillText('WT', x1, textY); // 在第一个柱状图上方标记 "Wt"
                ctx.fillText('Ebax KO', x2, textY); // 在第二个柱状图上方标记 "Nt"

                // 绘制 "EE" 文本标记在下划线下方
                const EETextWidth = ctx.measureText('EE').width;
                const EEX = ((x1 + x2) / 2) - EETextWidth / 2;
                ctx.fillText('Early embryo', EEX + 20, y + 20); // 在调整后的位置绘制 "EE"
                ...
                ctx.font = '14px Arial';
                ctx.fillText('WT', x3, textY); // 在第一个柱状图上方标记 "Wt"
                ctx.fillText('Ebax KO', x4, textY); // 在第二个柱状图上方标记 "Nt"

                // 绘制 "LE" 文本标记在下划线下方
                const LETextWidth = ctx.measureText('LE').width;
                const LEX = ((x3 + x4) / 2) - LETextWidth / 2;
                ctx.fillText('Late embryo', LEX + 12, y + 20); // 在调整后的位置绘制 "LE"
                ...
                ctx.font = '14px Arial';
                ctx.fillText('WT', x5, textY); // 在第一个柱状图上方标记 "WT"
                ctx.fillText('Ebax KO', x6, textY); // 在第二个柱状图上方标记 "Z8"

                // 绘制 "L1" 文本标记在下划线下方
                const L1TextWidth = ctx.measureText('L1').width;
                const L1X = ((x5 + x6) / 2) - L1TextWidth / 2;
                ctx.fillText('L1', L1X + 12, y + 20); // 在调整后的位置绘制 "L1"
                ...
                ctx.fillText('Ebax KO: Ebax Knockout', annotationX, annotationY + 40);

                // 绘制 "Data Source: GSE231447" 在最底部
                ctx.font = '12px Arial';
                ctx.fillText('Data Source: GSE267367',  0,  y+40); // 绘制普通文本
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            afterDraw: chart => {
                ...
                ctx.fillText('K562', K562X + 20, y + 20); // 在调整后的位置绘制 "K562"
                ...
                ctx.fillText('HeLa', HeLaX + 20, y + 20); // 在调整后的位置绘制 "HeLa"
                ...
                ctx.fillText('A549', A549X + 20, y + 20); // 在调整后的位置绘制 "A549"
                ...
                ctx.fillText('MCF7', MCF7X + 20, y + 20);
                ...
                ctx.fillText('HEK293T', HEK293TX + 20, y + 20);
            
                // In the top right corner add annotation
                const annotationX = chart.width - 200; // 右上角距离
                const annotationY = 50; // 从顶部开始的 Y 轴位置
                ctx.textAlign = 'left'; // 文本左对齐
                ctx.font = '14px Arial';
                ...
                // 绘制 "Data Source: GSE231447" 在最底部
                ctx.font = '12px Arial';
                ctx.fillText('Data Source: GSE158025, GSE160304', 0, y + 40); // 绘制普通文本
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                maxBarThickness: 150, // Limits the bar width to 150px
        ...
        options: {
            ...
            afterDraw: chart => {
                ...
                // 绘制 "Data Source: GSE231447" 在最底部
                ctx.font = '12px Arial';
                ctx.fillText('Data Source: GSE163388',  150,  y+40); // 绘制普通文本
